class preprocess(object):
    ...
    def get_valid_id(self, pre_data, fut_data):
        ...
        valid_id = []
        for idx in cur_id:
            exist_pre = [(False if isinstance(data, list) else (idx in data[:, 1])) for data in
                         pre_data[:self.min_past_frames]]
            exist_fut = [(False if isinstance(data, list) else (idx in data[:, 1])) for data in
...
from data.nuscenes.map import GeometricMap
